...
class Transaction_model extends CI_Model {
    ...
    public function getOrder($id){
        ...
        if($data){
            ...
            $this->db->join("books", "books.book_id = transaction_detail.book_id");
        ...
        if($order){
            if(in_array($order["transaction_status"], ["Menunggu pembayaran", "Pembayaran diterima"]) && !in_array($status, ["Menunggu pembayaran", "Pembayaran diterima"])){
                ...
                foreach($order["details"] as $detail){
                    $stok = $detail["book_stok"] - $detail["detail_qty"];
                    $this->db->update("books", ["book_stok"=>$stok], ["book_id"=>$detail["book_id"]]);
                }
            }
    ...
    public function orderDetail($transaction_id, $book_id, $detail_qty, $detail_price){
